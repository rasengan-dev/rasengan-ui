...
export const BookingCard3 = ({ className }: Props) => {
	return (
		<article
			className={twMerge(
				"w-full max-w-[400px] rounded-xl p-1 shadow-md bg-background dark:bg-input/40 overflow-hidden",
				className
			)}
		>
						...
						{Array.from({ length: 4 }).map((_, index) => (
							<CarouselItem key={index}>
								<div className='relative p-0'>
									<Image
										src={`/static/images/booking/${index + 1}.jpeg`}
										alt='Avatar'
									...
									<div className='absolute inset-0 bg-black/50 p-4 rounded-xl flex justify-between'>
										<div>
											<Badge
												variant={"secondary"}
